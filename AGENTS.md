...
- Honest storage (P0-07 done, 2026-08-19): all false security claims removed ("AES-256", "256-Bit Encrypted", "End-to-end encrypted", "ZERO ACCESS", "Secure Connection", base64-as-encryption) — chat/memories are now plaintext JSON in localStorage with honest labels; fabricated default memories no longer injected; AI memory sharing requires **explicit consent** (`getAiMemoryConsent`/`setAiMemoryConsent`, default OFF) wired into the coach payload, the Memory panel toggle, and the Second Brain toggle. Enforced by `server/noFalseSecurityClaims.test.ts`.
- PDF export hardened (P0-08 done, 2026-08-19): `exportThreadToPDF` no longer uses `document.write`/HTML string interpolation — the print document is built with DOM APIs and every user-controlled value (thread title, message text) is assigned via `textContent` (auto-escaped). A message containing `<script>` renders as literal text. Enforced by `src/lib/chatStorage.export.test.ts` (drives the real function with a fake DOM that has no innerHTML/document.write support).
- Hardcoded profile PII removed (P0-11 done, 2026-08-20): a real person's identity (personal Gmail, names, phone numbers, DOB, emergency contacts, family members) was baked in as app defaults — OTA attribution fallback, demo transaction seed, payment/profile modal defaults, dashboard/leaderboard/tweak-wall/voice-preview strings, and a hidden `handlePresetAcceptanceTests` coach-chat backdoor that answered personal-name queries with scripted fake "Gemini" responses claiming to have logged the profile. All removed: OTA deploy/rollback now attribute to the verified admin identity (`req.user?.email || deployedBy || 'admin'`), the demo transaction seed uses `demo.user@example.com`/`Demo User`, profile state starts empty, the leaderboard shows "You", and the preset backdoor is deleted (coach chat always reaches the real flow). A follow-on audit also scrubbed the name from ``Founder & Creator`` lines (`App.tsx` footer, `HamburgerMenu` about panel, `metadata.json`), `prisma/seed.ts`, and `supabase_schema.sql` (demo transaction + admin-promotion example). Enforced by `server/noHardcodedPII.test.ts` (scans the UI tree plus `prisma/seed.ts`, `supabase_schema.sql`, `metadata.json`; rejects every banned string).
